...
/**
 * Return the given `words` mapped to the  constant.
 *
 * Examples:
 *
 *    (['tobi', 'wants', '4', 'dollars'])
 *    // => { '4': '4', tobi: 'tobi', wants: 'wants', dollars: 'dollars' }
 *
 * @param {Array} words
 * @return {Object}
 * @api private
...
exports.wordMap = function(words){
  var obj = {};
  for (var i = 0, len = words.length; i < len; ++i) {
    obj[words[i]] = words[i];
  }
...
/**
 * Return an array of  constants in `words`.
 *
 * Examples:
 *
 *    (['tobi', 'wants', '4', 'dollars'])
 *    // => ['4', 'tobi', 'wants', 'dollars']
 *
 * @param {Array} words
 * @return {Array}
 * @api private
...
exports.wordArray = function(words){
  var arr = []
    , constant;
  for (var i = 0, len = words.length; i < len; ++i) {
    constant = words[i];
...
/**
 * Return a map of  constant redis keys for `words`
 * and the given `key`.
 *
 * @param {String} key
 * @param {Array} words
...
exports.wordKeys = function(key, words){
  return exports.wordArray(words).map(function(c){
    return key + ':word:' + c;
  });
};
...
Query.prototype.end = function(fn){
  var key = this.search.key
    ...
    , keys = exports.wordKeys(key, words)
    , type = this._type;
