...
var generateUrls = exports.generateUrls = {
    get : function() {
        var urls = [];
        var template1 = 'craigslist.org/';
        var template2 = '/index.rss';
...
var jasonparser = exports.jasonparser = {
    ...
    parseandstoreredis : function(content) {
        var parser = new xml2js.Parser({
            explicitArray : true
        });
        parser.addListener('end', function(result) {
            storagengineredis.store(result, function(done) {
            });
        });
        if (content != null) {
            parser.parseString(content);
    ...
    parseandstorelastic : function(content) {
        var parser = new xml2js.Parser({
            explicitArray : true
        });
        parser.addListener('end', function(result) {
            storagenginelastic.store(result, function(done) {
            });
        });
        if (content != null) {
            parser.parseString(content);
...
var storagengineredis = exports.storagengineredis = {
...
var storagenginelastic = exports.storagenginelastic = {
    store : function(result,callback) {
        if(result.item != undefined) {
            for(var i in result.item) {
                city = getCity(result.item[i].link[0]);
                ...
                // I am going to change this to the posting ID which is part of the URL
                json.uuid = Math.floor(Math.random() * 9999);
                writeJsonToElastic(json.url,json, function(done) {
                });
            }
        }
    }
        ...
        replies.forEach(function(reply, i) {
            _.each(categories, function(category) {
                client.hget(reply, category, function(err, content) {
                    ...
                    jasonparser.parseandstoreredis(content);
                });
            })
        });
    });
        ...
        replies.forEach(function(reply, i) {
            _.each(categories, function(category) {
                client.hget(reply, category, function(err, content) {
                    ...
                    var content = pipeline.parser(content);
                    jasonparser.parseandstorelastic(content);
                });
            })
        });
    });
...
function writeJsonToElastic(url,content, callback) {
    city = getCity(url);
    console.log('writeJsonToElastic ' + city);
    client.publish('action', JSON.stringify(content), function(err, result) {
        callback(result);
    });
...
if(process.argv[1] === __filename) {
    ...
    if(argv.r) {
        ...
        console.log('copying data from rss to json in redis');
        deleterediskeys(function(done){});
        rsstojsonredis();
    }
    else if (argv.e){
        console.log('copying data from rss to json in elasticsearch');
        deleterediskeys(function(done){});
        rsstojsonelastic();
    }
    else if (argv.j){
